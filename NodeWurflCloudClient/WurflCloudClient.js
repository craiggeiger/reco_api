...
//constructor
var WurflCloudClient = function(config, user_request, user_response){
    this.config = config;
...
WurflCloudClient.prototype = {
	...
	this.cache.getDevice(http_request, function(){
	    ...
	    if (!result) { //no cookie cache
		thiss.source = this.SOURCE_CLOUD;
		var that = thiss;
	    ...
	    }else{ //found cookie cache
		thiss.source = thiss.SOURCE_CACHE;
		thiss.capabilities = result;
		if (!thiss._allCapabilitiesPresent()){ //all search_capabilities were not found in cookie cache
		...
		}else{  //all search_capabilities found in cookie cache
		    return callback(null, thiss.capabilities);
		}
	    }
	});
     ...
     */
    _initializeRequest: function(){
	this._splitApiKey();
	//reportAge etc functions ommitted...
	...
	if (this.loaded_date === null){
	    ...
	    lastmod = this.cache.lastModified;
	    this.loaded_date = Date.parse(lastmod);
	}
...
// addReortFataToRequest omitted here...
// validateCache ommitted too...
	...
	if (this.source === this.SOURCE_CLOUD){
	    this.cache.setDevice(this.capabilities);
	}
    },
	...
	// FIXME: Implement v1.5 cache key logic
	if (source === null){
